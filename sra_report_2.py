...
test_data = pd.read_excel('/mnt/e/training_data/zxjh/zxjh-training-data/zxjh_testing data set_202001.xlsx', engine="openpyxl", skiprows=3)
...
predict = pd.read_excel("/mnt/e/training_data/zxjh/predictions/sample.xlsx", engine="openpyxl", sheet_name="result")
...
plt.savefig('reports/chart1.png')
...
plt.tight_layout()
plt.savefig('reports/renyin.png')

plt.figure(figsize=(19,12))
type2.plot(kind='barh',title="物因")
plt.tight_layout()
plt.savefig('reports/wuyin.png')

plt.figure(figsize=(19,12))
type3.plot(kind='barh',title="环境")
plt.tight_layout()
plt.savefig('reports/huanjing.png')

plt.figure(figsize=(19,12))
type4.plot(kind='barh',title="管理")
plt.tight_layout()
plt.savefig('reports/guanli.png')
...
writer = pd.ExcelWriter("reports/整改措施评定.xlsx")
...
plt.tight_layout()
plt.savefig("reports/chart3.png")
...
plt.savefig("reports/risk_level1.png")
...
r_values.plot(kind="pie",autopct="%1.1f%%",title="机判断风险级别"
              ,colors=['red','orange','yellow','lightskyblue'])
plt.savefig("reports/risk_level2.png")
...
writer=pd.ExcelWriter("reports/员工错误题目.xlsx")
